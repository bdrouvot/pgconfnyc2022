...
do
  ...
  dsk_xmin=`dd status=none bs=8192 count=1 if=$relpath skip=$blk | od -A n -t u4 -j $dsk_lp_off -N 4`
  toread=$(( 4 + $dsk_lp_off ))
  dsk_xmax=`dd status=none bs=8192 count=1 if=$relpath skip=$blk | od -A n -t u4 -j $toread -N 4`
  toread=$(( 8 + $dsk_lp_off ))
...
from_mem=`psql -tA -c "SELECT lp,lp_off,lp_flags,lp_len,t_xmin,t_max,t_field3,t_ctid,t_infomask2,t_infomask,t_hoff,t_bits,t_oid FROM heap_page_items(get_raw_page('$tb', $blkmem))"`
